...
namespace MyApi.Controllers.v1
{
    public class OldPostsController : BaseController
    {
        ...
        public async Task<ActionResult<List<PostSelectDto>>> Get(CancellationToken cancellationToken)
        {
            ...
            var list = await _mapper.ProjectTo<PostSelectDto>(_repository.TableNoTracking
        ...
        {
            var dto = await _mapper.ProjectTo<PostSelectDto>(_repository.TableNoTracking).SingleOrDefaultAsync(p => p.Id == id, cancellationToken);
        ...
        public async Task<ApiResult<PostSelectDto>> Create(PostDto dto, CancellationToken cancellationToken)
        {
            ...
            var resultDto = await _mapper.ProjectTo<PostSelectDto>(_repository.TableNoTracking).SingleOrDefaultAsync(p => p.Id == model.Id, cancellationToken);
